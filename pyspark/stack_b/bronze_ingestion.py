...
from pyspark.sql.functions import col, current_timestamp, md5, concat_ws, lit, row_number
...
class BronzeLayerIngestor:
    ...
    def ingest_customers(self, csv_path: str, load_date: str) -> int:
        ...
        try:
            df = (
                self.spark.read.schema(self.get_customers_schema())
                .option("header", "true")
                .csv(csv_path)
            )
            ...
            df.write.format("delta").mode("overwrite").option("mergeSchema", "true").save(
                f"{self.bronze_location}/customers"
            )
    ...
    def ingest_products(self, csv_path: str, load_date: str) -> int:
        ...
        try:
            df = (
                self.spark.read.schema(self.get_products_schema())
                .option("header", "true")
                .csv(csv_path)
            )
            ...
            df.write.format("delta").mode("overwrite").option("mergeSchema", "true").save(
                f"{self.bronze_location}/products"
            )
    ...
    def ingest_transactions(self, csv_path: str, load_date: str) -> int:
        ...
        try:
            df = (
                self.spark.read.schema(self.get_transactions_schema())
                .option("header", "true")
                .csv(csv_path)
            )
            ...
            df.write.format("delta").mode("overwrite").option("mergeSchema", "true").save(
                f"{self.bronze_location}/transactions"
            )
...
def main():
    ...
    try:
        results = {
            ...
            "transactions": ingestor.ingest_transactions(
                f"{SOURCE_DATA_PATH}/transactions.csv", load_date
            ),
        }
