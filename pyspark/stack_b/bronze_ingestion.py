...
from pyspark.sql.functions import (
    col, current_timestamp, md5, concat_ws, lit, row_number
)
    ...
    def validate_bronze_quality(self, table_name: str) -> dict:
        ...
        try:
            df = self.spark.read.format("delta") \
                .load(f"{self.bronze_location}/{table_name}")
    ...
    try:
        results = {
            'customers': ingestor.ingest_customers(
                f"{SOURCE_DATA_PATH}/customers.csv", load_date),
            'products': ingestor.ingest_products(
                f"{SOURCE_DATA_PATH}/products.csv", load_date),
