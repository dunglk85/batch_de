...
from pyspark.sql.functions import (
    col, to_date, current_timestamp, count, sum, avg, when, lit, min, max,
    round, datediff, row_number
)
...
class GoldLayerAggregator:
    ...
    def build_daily_sales_fact(self, load_date: str) -> int:
        ...
        df = tx.join(prd, "product_id", "left") \
            ...
            .groupBy("txn_date", "customer_id", "product_id", "category",
                     "store_location") \
            .agg(
                sum("quantity").alias("quantity"),
                sum(when(col("status") == "completed", col("amount"))
                    .otherwise(lit(0))).alias("gross_amount"),
                sum(col("amount")).alias("net_amount"),
                count("*").alias("transaction_count"),
                count(when(col("status") == "completed", 1))
                .alias("completed_count"),
                count(when(col("status") == "refunded", 1))
                .alias("refunded_count"),
            ) \
    ...
    def build_customer_metrics(self, load_date: str) -> int:
        ...
        preferred_payment = tx.groupBy("customer_id", "payment_method") \
            ...
            .withColumn("rn", row_number().over(
                Window.partitionBy("customer_id")
                .orderBy(col("pm_count").desc()))) \
            .filter(col("rn") == 1) \
            .select("customer_id",
                    col("payment_method").alias("preferred_payment_method"))
        ...
        df = prd \
            ...
            .withColumn("inventory_turnover_ratio",
                        when(col("stock_quantity") > 0,
                             round(col("total_quantity_sold")
                                   / col("stock_quantity"), 2))
                        .otherwise(lit(0))) \
            .withColumn("is_profitable",
                        when(col("total_revenue")
                             > col("price") * col("total_quantity_sold") * 0.7,
                             lit(True))
                        .otherwise(lit(False))) \
            .withColumn("_updated_at", current_timestamp()) \
            .withColumn("_load_date", lit(load_date))
    ...
    def build_category_trends(self, load_date: str) -> int:
        ...
        window_7d = Window.partitionBy("category") \
            .orderBy("trend_date").rowsBetween(-6, 0)
        df = daily \
            .withColumn("moving_avg_7_days",
                        round(avg("daily_sales").over(window_7d), 2)) \
            .withColumn("_updated_at", current_timestamp()) \
            .withColumn("_load_date", lit(load_date))
...
def main():
    ...
    try:
        ...
        for name in results:
            spark.read.format("delta").load(f"{GOLD}/{name}") \
                .createOrReplaceTempView(f"gold_{name}")
